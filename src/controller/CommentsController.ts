...
import { GetCommentsForPostSchema } from "../dtos/comments/getCommentsDorPost.dto";
...
export class CommentsController {
  ...
  public getCommentsForPost = async (req: Request, res: Response) => {
    try {
      const input = GetCommentsForPostSchema.parse({
        postId: req.params.postId,
        token: req.headers.authorization,
      });

      const output = await this.commentsBusiness.getCommentsForPost(input);
  ...
  try {
    const input = EditCommentSchema.parse({
      ...
      newContent: req.body.content, // Corrigir o nome da propriedade para 'newContent'
      token: req.headers.authorization,
    });
